...
def technique_output_hint(name: str) -> str:
    m = {
        ...
        "Scanning Electron Microscopy": "Surface topograpy and Chemistry",
...
METHOD_HOURS = {
    ...
    "XRF": 1.5,
    "XRD": 1.5,
    "Scanning Electron Microscopy": 3.0,
...
def editor_with_excel_paste(
    *,
    ...
    """
    A Streamlit data editor component that includes a text area for pasting data from Excel.
    Manages DataFrame state in st.session_state.
    """
    store_key = f"{editor_key}__df"
    if store_key not in st.session_state:
        st.session_state[store_key] = default_df.copy()

    df_current = st.session_state[store_key]

    def paste_ui():
        st.markdown(f"**{label}**")
        txt = st.text_area("Paste copied Excel range here", key=paste_key, height=120)
        if st.button(apply_button_text, key=f"{paste_key}_apply"):
            imported = parse_excel_paste(txt)
            if imported is not None and isinstance(imported, pd.DataFrame) and not imported.empty:
                st.session_state[store_key] = imported
                st.rerun() # Rerun to show the updated table in the editor
            else:
                st.warning("Import failed. Please copy a valid table range from Excel/Sheets and try again.")

    if use_expander:
        with st.expander("📋 Import from Excel/Sheets"):
            paste_ui()
    else:
        paste_ui()
...
def comparison_matrix_editor(
    *,
    ...
    - Optional import from Excel clipboard in an expander
    - Add column button
    ...
    if c1.button("➕ Add column", key=f"{editor_key}__add_col"):
        ...
        df_current[new_col] = ""
        st.session_state[store_key] = df_current # Update session state immediately

    # Remove selected column
    if len(df_current.columns) > 1: # Don't allow removing the last column
        column_to_remove = c2.selectbox(
            "Column to remove",
            options=list(df_current.columns),
            key=f"{editor_key}__remove_col_select",
            label_visibility="collapsed"
        )
        if c3.button("🗑️ Remove column", key=f"{editor_key}__remove_col_btn"):
            if column_to_remove in df_current.columns:
                df_current = df_current.drop(columns=[column_to_remove])
                st.session_state[store_key] = df_current # Update session state immediately


    # Optional import (hidden/compact UI)
    with c2.expander("📋 Import from Excel", expanded=False):
    ...
    # ---- Table editor ----
    st.markdown("💡 **Tip:** To remove a column, select its header and press the `Delete` key.")
    df_out = st.data_editor(
        df_current,
        num_rows="dynamic",
        use_container_width=True,
...
def main():
    ...
    if "pdf_download_bytes" in st.session_state:
        pdf_bytes = st.session_state.pdf_download_bytes
        sample_id = st.session_state.get("sample_id_for_filename", "report")
        ...
        del st.session_state["pdf_download_bytes"]
        if "sample_id_for_filename" in st.session_state:
            del st.session_state["sample_id_for_filename"]
    ...
    with st.container():
        ...
        revision = c5.text_input("Revision", "0")
    ...
    with col2:
        technique_flags["Scanning Electron Microscopy"] = st.checkbox("Scanning Electron Microscopy")
    ...
    SECTION_TITLES = ["2. Analysis results", "3. Discussion", "4. Experimental"]
    selected_main_section_title = st.selectbox(
        "Select Main Section Title for Results",
        options=SECTION_TITLES,
        key="main_section_title"
    )
    ...
    if technique_flags["ICP-OES"]:
        with st.expander("🧪 ICP-OES Data", expanded=True):
            ...
            mass = c_a.number_input("Mass (g)", 0.100, format="%.3f", key="icp_mass")
            volume = c_a.number_input("Volume (mL)", 500, format="%.3f", key="icp_volume")

            st.markdown("##### Data Table")
            df_icp = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="icp_main_table",
                label="ICP Data Table",
            )

            st.markdown("##### Images (optional)")
            icp_files = st.file_uploader("Upload Images", accept_multiple_files=True, key="icp_files")
            icp_captions = st.text_area("Image Captions (one per line)", key="icp_captions")
    ...
    if technique_flags["XRF"]:
        with st.expander("🧪 XRF Data", expanded=True):
            st.markdown("##### Data Table")
            df_xrf = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="xrf_main_table",
                label="XRF Data Table",
            )

            st.markdown("##### Images (optional)")
            xrf_files = st.file_uploader("Upload Images", accept_multiple_files=True, key="xrf_files")
            xrf_captions = st.text_area("Image Captions (one per line)", key="xrf_captions")
    ...
    if technique_flags["Scanning Electron Microscopy"]:
        with st.expander("🔬 Scanning Electron Microscopy Data", expanded=True):
            c_a, c_b = st.columns(2)
            kv = c_a.number_input("Voltage (kV)", 20, key="sem_kv")
            wd = c_b.number_input("Working Dist (mm)", 10.0, key="sem_wd")

            st.markdown("##### Data Table")
            df_sem = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="sem_main_table",
                label="SEM-EDX Data Table",
            )

            st.markdown("##### Images (optional)")
            sem_files = st.file_uploader("Upload SEM Images", type=['jpg', 'png', 'tif'], accept_multiple_files=True, key="sem_files")
            sem_captions = st.text_area("Image Captions (one per line)", key="sem_captions")

            st.divider()
            method_ref, operator, op_notes = add_operator_fields(st.container(), key_prefix="SEM-EDX", method_name="SEM-EDX")
            report_data["Scanning Electron Microscopy"] = {
    ...
    if technique_flags["Optical"]:
        with st.expander("📷 Optical Microscopy", expanded=True):
            etchant = st.text_input("Etchant Used", "Nital 3%", key="opt_etchant")

            st.markdown("##### Data Table")
            df_opt = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="opt_main_table",
                label="Optical Microscopy Data Table",
            )

            st.markdown("##### Images (optional)")
            opt_files = st.file_uploader("Upload Optical Images", type=['jpg', 'png'], accept_multiple_files=True, key="opt_files")
            opt_captions = st.text_area("Image Captions (one per line)", key="opt_captions")
    ...
    if technique_flags["PSD"]:
        with st.expander("📊 Particle Size Distribution", expanded=True):
            dispersant = st.text_input("Dispersant agent", key="psd_dispersant")

            st.markdown("##### Data Table")
            df_psd = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="psd_main_table",
                label="PSD Data Table",
            )

            st.markdown("##### Images (optional)")
            psd_files = st.file_uploader("Upload Images", accept_multiple_files=True, key="psd_files")
            psd_captions = st.text_area("Image Captions (one per line)", key="psd_captions")


            st.divider()
            method_ref, operator, op_notes = add_operator_fields(st.container(), key_prefix="PSD", method_name="PSD")
            report_data["PSD"] = {
                "meta": f"Dispersant: {dispersant}",
                "table": df_psd,
                "images": psd_files,
                "captions": psd_captions,
                "method_ref": method_ref,
                "operator": operator,
                "op_notes": op_notes
            }
    ...
    if technique_flags["XRD"]:
        with st.expander("XRD", expanded=True):
            st.markdown("##### Data Table")
            df_xrd = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="xrd_main_table",
                label="XRD Data Table",
            )

            st.markdown("##### Images (optional)")
            xrd_files = st.file_uploader("Upload XRD result", type=['jpg', 'png'], accept_multiple_files=True, key="xrd_files")
            xrd_captions = st.text_area("Image Captions (one per line)", key="xrd_captions")
    ...
    if technique_flags["TGA"]:
        with st.expander("TGA", expanded=True):
            st.markdown("##### Data Table")
            df_tga = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="tga_main_table",
                label="TGA Data Table",
            )

            st.markdown("##### Images (optional)")
            tga_files = st.file_uploader("Upload TGA result", type=['jpg', 'png'], accept_multiple_files=True, key="tga_files")
            tga_captions = st.text_area("Image Captions (one per line)", key='tga_captions')
    ...
    if technique_flags["BET"]:
        with st.expander("BET", expanded=True):
            surface_area = st.number_input("Surface area (m²/g)", key="bet_surface_area")

            st.markdown("##### Data Table")
            df_bet = comparison_matrix_editor(
                default_df=default_comparison_df(),
                editor_key="bet_main_table",
                label="BET Data Table",
            )

            st.markdown("##### Images (optional)")
            bet_files = st.file_uploader("Upload BET result", type=['jpg', 'png'], accept_multiple_files=True, key="bet_files")
            bet_captions = st.text_area("Image Captions (one per line)", key="bet_captions")

            st.divider()
            method_ref, operator, op_notes = add_operator_fields(st.container(), key_prefix="BET", method_name="BET")

            report_data["BET"] = {
                "meta": f"Surface Area: {surface_area} m²/g",
                "table": df_bet,
                "images": bet_files,
                "captions": bet_captions,
                "method_ref": method_ref,
                "operator": operator,
                "op_notes": op_notes
            }
    ...
    if technique_flags["Porosity"]:
        with st.expander("🫧 Porosity", expanded=True):
            ...
            st.markdown("##### Data Table")
            df_por = comparison_matrix_editor(
                default_df=default_porosity_df(),
                editor_key="por_main_table",
                label="Porosity Data Table",
            )

            st.markdown("##### Images (optional)")
            por_files = st.file_uploader("Upload Porosity Images", type=["jpg", "jpeg", "png", "tif", "tiff"], accept_multiple_files=True, key="por_files")
            por_captions = st.text_area("Image Captions (one per line)", key="por_captions")
    ...
    if technique_flags["Al Grain Size"]:
        with st.expander("🌾 Al Grain Size (GSA)", expanded=True):
            ...
            st.markdown("##### Data Table")
            df_gsa = comparison_matrix_editor(
                default_df=default_gsa_df(),
                editor_key="gsa_main_table",
                label="GSA Data Table",
            )

            st.markdown("##### Images (optional)")
            gsa_files = st.file_uploader("Upload Grain Size Images", type=["jpg", "jpeg", "png", "tif", "tiff"], accept_multiple_files=True, key="gsa_files")
            gsa_captions = st.text_area("Image Captions (one per line)", key="gsa_captions")
    ...
    if chem_selected:
        with st.expander("🧪 Chemical analysis", expanded=True):
            ...
            for tab, method in zip(tabs, chem_selected):
                with tab:
                    st.markdown("##### Data Table")
                    df_chem = comparison_matrix_editor(
                        default_df=default_comparison_df(),
                        editor_key=f"chem_{safe_key(method)}_main_table",
                        label=f"{method} Data Table",
                    )

                    st.markdown("##### Images (optional)")
                    chem_files = st.file_uploader("Upload Images", accept_multiple_files=True, key=f"chem_{safe_key(method)}_files")
                    chem_captions = st.text_area("Image Captions (one per line)", key=f"chem_{safe_key(method)}_captions")
    ...
    if st.button("🚀 Generate PDF Report", type="primary"):
        ...
        technique_order = [
            ...
            "Scanning Electron Microscopy", "Optical", "Porosity",
            "PSD", "BET", "XRD", "TGA",
        ...
        pdf.section_header_keep(st.session_state.main_section_title, first_block_mm=35.0)

        # D. Loop through results
                # --- Non-chemical results ---
        main_section_number = st.session_state.main_section_title.split('.')[0] # Extract "2" from "2. Analysis results"
        result_index = 1
        ...
        for tech in techniques_included:
            ...
            pdf.subtitle(f"{main_section_number}.{result_index} {tech}")
            result_index += 1
        ...
        if chem_items:
            ...
            pdf.subtitle(f"{main_section_number}.{result_index} Chemical analysis")
            result_index += 1
            ...
            for method_name, data in chem_items:
                ...
                pdf.set_font(theme.font_main, "B", 10)
                pdf.set_text_color(*theme.text_dark)
                pdf.cell(0, 6, f"{main_section_number}.{result_index-1}.{sub_i} {method_name}", 0, 1, "L")
                sub_i += 1
        ...
        try:
            ...
            st.session_state.sample_id_for_filename = sample_id
            st.rerun()
